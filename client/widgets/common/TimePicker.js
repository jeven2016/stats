...
export default class TimePicker extends Component {
    ...
    getMenu(type, limitValue) {
        ...
        for (let i = 0; i < limitValue; i++) {
            ...
            nodeItem = (<li key={key} className={currentClass} onClick={this.onItemSelected.bind(this, type, i)}>
                {displayValue}
            </li>);
            items.push(nodeItem);
        }
    ...
    getDisplayMomentTime() {
        ...
        displayMomentTime = displayMomentTime ? displayMomentTime : ( value ? value : defaultTime);
    ...
    render() {
        ...
        let inlineClass = this.props.isInline ? "time-container icon-input" : "icon-input";
        ...
        if (value) {
            icon = <i className="fa fa-remove icon" onClick={this.resetTime.bind(this)}/>
        }
