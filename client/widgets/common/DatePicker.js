...
class DateHelper {
    static getNumberOfDays(momentDate) {
        ...
        switch (month) {
            case 2: {
                //能被4整除不能被100整除的就是闰年，能被400整除的年份也是闰年
...
export default class DatePicker extends Component {
    ...
    createColumns(momentDate) {
        ...
        //for last month
        let daysOfLastMonth = DateHelper.getNumberOfDays(moment({y: momentDate.year(), m: momentDate.month() - 1}));
        ...
        for (let i = 0; i < leftLen; i++) {
            ...
            td = (<td key={key}>
                <button className="button text comment" key={i + 1}
                        onClick={this.onItemSelected.bind(this, {month: month + 1, day: i + 1})}>{i + 1}</button>
            </td>);
            columns.push(td);
        }
    ...
    getDisplayMomentDate() {
        ...
        displayMomentDate = displayMomentDate ? displayMomentDate : ( value ? value : defaultDate);
    ...
    render() {
        ...
        if (value) {
            icon = <i className="fa fa-remove icon" onClick={this.resetDate.bind(this)}/>
        }
